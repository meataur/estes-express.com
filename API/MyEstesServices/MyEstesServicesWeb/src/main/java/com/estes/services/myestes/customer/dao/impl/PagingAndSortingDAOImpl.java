...
public class PagingAndSortingDAOImpl <T> implements PagingAndSortingDAO<T> {
	...
	public Page<T> getResult(String countQuery,String query, List<Object> params, Pageable pageable, RowMapper<T> rowMapper) throws ServiceException{
		...
		if(page < 1){
			throw new ServiceException("Invalid page number. Page number should start with 1");
		}
		...
		if(page < 1){
			throw new ServiceException("Invalid page number. Page number should not exceed total page number");
		}
