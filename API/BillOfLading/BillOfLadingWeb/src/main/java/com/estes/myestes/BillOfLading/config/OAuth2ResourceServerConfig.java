...
public class OAuth2ResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(final HttpSecurity http) throws Exception {
		...
		.and().authorizeRequests()
				...
				.antMatchers("/accessorials").permitAll()
				.antMatchers("/user/pickup_information").access("#oauth2.hasScope('"+env.getProperty("app.id.pickuprequest")+"')")
		.and().authorizeRequests()
				.anyRequest().access("#oauth2.hasScope('"+env.getProperty("app.id.bol")+"')");
	}
