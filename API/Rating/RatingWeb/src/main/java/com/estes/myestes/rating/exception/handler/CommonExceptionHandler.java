...
public class CommonExceptionHandler {
	...
	protected ResponseEntity<?> handleNoHandlerFoundException(Exception ex, WebRequest request) {
		...
		ESTESLogger.log(ESTESLogger.ERROR, ex.getStackTrace()[0].getClassName(), ex.getStackTrace()[0].getMethodName(),
				ex.getMessage() +
				"\n Request Details: " + request.getDescription(true));
	...
	protected ResponseEntity<?> handleDataTruncation(DataTruncation ex, WebRequest request) {
		...
		ESTESLogger.log(ESTESLogger.DEBUG, ex.getStackTrace()[0].getClassName(), ex.getStackTrace()[0].getMethodName(),
				ex.getMessage() +
				"\n Request Details: " + request.getDescription(true)
				+"\n\n The index of the parameter or column value: "+ ex.getIndex()
				+"\n the original size of the data: "+ex.getDataSize()
				+"\n the size after truncation : "+ex.getTransferSize()
				+"\n parameter value  truncated? :  "+ex.getParameter()
				+ "a read truncated:? "+ex.getRead()
				);
		return new ResponseEntity<>(error, HttpStatus.INTERNAL_SERVER_ERROR);
	}
